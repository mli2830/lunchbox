...
simspaceCB <- function(spacenum = 1,betatemp = 0.02, N=100, effprop=0.9, i0=1,
  ...
  tvec <- seq(1,numobs)
  n <- length(tvec)
  S <- I <- matrix(0,ncol=spacenum,nrow=numobs)
  R <- Iobs <- numeric(numobs)
  pSI <- array(0,dim=c(numobs,spacenum,spacenum))
  ...
  pSI[1,1,1] <- 1 - (1-beta[1,1,1])^I[1,1]
  ...
  for (t in 2:n){
    for (i in 1:spacenum){
      for (j in 1:spacenum){
        I[t,i] <- rbinom(1,prob=pSI[t-1,i,j],size=S[t-1,i])
